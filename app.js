...
const topicSeedData = [
  ["Caf茅 de madrugada", "Discusi贸n abierta para la ronda de la noche."],
  ["Balance del clan", "Ideas para ajustar econom铆a y roles."],
  ["Eventos del domingo", "Armen propuestas y horarios."],
  ["Memes internos", "Solo material del servidor, sin compilar ajeno."],
  ["Gu铆as y rutas", "Tips cortos para gente nueva."],
  ["Mercado local", "Intercambios, ventas y reservas."],
  ["Encuentros RP", "Coordinaci贸n de escenas y personajes."],
  ["Ajustes del HUD", "Feedback visual y comodidad."],
  ["Arte y capturas", "Im谩genes del juego y fanart."],
  ["Combate y builds", "Probamos configuraciones y counters."],
  ["Preguntas r谩pidas", "Espacio para dudas cortas."],
  ["Buz贸n de ideas", "Mejoras, sugerencias y experimentos."],
  ["Historias del server", "Recuerdos, an茅cdotas y episodios."],
  ["M煤sica de fondo", "Listas para acompa帽ar partidas largas."],
  ["Alianzas", "Negociaciones entre facciones."],
  ["Torneo semanal", "Inscripci贸n, brackets y resultados."],
  ["Novedades", "Cambios recientes y anuncios suaves."],
  ["C贸digos y macros", "Atajos, bindings y trucos 煤tiles."],
  ["Radio interna", "Temas de charla para el descanso."],
  ["Cocina del caf茅", "Lo que se sirve cuando baja la intensidad."]
];

const initialUsers = [
  { id: "u1", name: "Coco Mora", role: "T煤", score: 142 },
  { id: "u2", name: "Nadia", role: "Moderaci贸n", score: 129 },
  { id: "u3", name: "Rulo", role: "Veterano", score: 117 },
  { id: "u4", name: "Mara", role: "Dise帽o", score: 103 },
...
function buildTopics() {
  return topicSeedData.map(([title, subtitle], index) => {
    ...
    const messages = [
      createMessage(authorSet[index % authorSet.length], `${title} qued贸 abierto para coordinar sin ruido.` , 48),
      ...
      createMessage(authorSet[(index + 3) % authorSet.length], `Si hace falta, lo seguimos ac谩 mismo.`, 8)
    ];
...
function bindEvents() {
  ...
  document.getElementById("storeButton").addEventListener("click", () => {
    flashTitle("Tienda en preparaci贸n");
  });
...
function renderTopics(targetId = "topicList") {
  ...
  state.topics.slice(0, 20).forEach((topic, index) => {
    ...
    const lastCommenterName = lastCommentAuthor ? getUser(lastCommentAuthor.authorId)?.name ?? "An髇imo" : "Sin actividad";
    button.innerHTML = `
...
function renderMessage(message) {
  ...
  const author = message.kind === "system" ? "Sistema" : getUser(message.authorId)?.name ?? "An贸nimo";
...
function renderUsers(targetId = "userList") {
  ...
  ordered.forEach((user) => {
    ...
    item.innerHTML = `
      ...
      <div class="user-item__meta">${escapeHtml(user.role)} 路 ${user.online ? "Conectado" : "Offline"} 路 ${user.score} pts</div>
    `;
    ...
    return [
      {
        id: "empty",
        title: "Sin actividad",
        meta: "Todav铆a no hay participaci贸n en este tema.",
        active: false
      }
    ];
  }
...
function refreshCurrentTopic() {
  ...
  if (!last || last.kind !== "system" || state.refreshCount % 3 === 0) {
    topic.messages.push(
      createMessage(
        "u2",
        `Actualizaci髇 manual ${state.refreshCount}: la sala sigue estable y sin sincronizaci髇 en tiempo real.`,
        0,
        "system"
      )
    );
